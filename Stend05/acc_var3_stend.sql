...
ALTER SESSION SET TRACEFILE_IDENTIFIER = "ACC_VAR3";
...
-- start   
     SELECT dd.acc, dd.dt, dd.summ bulk collect into v_table 
       FROM (SELECT /*+ index(t IX_TABLE1_DT)*/ t.acc, t.dt, MAX(dt) KEEP (DENSE_RANK LAST ORDER BY dt) OVER (PARTITION BY t.acc) AS max_dt, t.summ
               FROM rf.table1 t
